...
void rxnoutput(simptr sim,int order) {
	...
	for(r=0;r<rxnss->totrxn;r++) {
		...
		if(order==2 && rxn->difflimit>0) simLog(sim,2,"   requested diffusion-limited amount: %g\n",rxn->difflimit);
...
/*			if(rxn->rparamt!=RPconfspread && rxn->rparamt!=RPbounce && findreverserxn(sim,order,r,&o2,&r2)==1) {
				...
				rparamt=rxn->rparamt;
				rparam=rxn->rparam;
				if(rparamt==RPunbindrad) bindradrev=bindingradius(revrate,0,revdsum,rparam,0);		//?? should update to bindingradiusprob
				else if(rparamt==RPratio) bindradrev=bindingradius(revrate,0,revdsum,rparam,1);		//?? should update to bindingradiusprob
				else if(rparamt==RPpgem || rparamt==RPpgemmax || rparamt==RPpgemmaxw) {
					bindradrev=bindingradiusrev(revrate*(1.0-rparam),0,revdsum,-1,0);									//?? should update to bindingradiusprob
					pgem=rparam; }
				else bindradrev=bindingradius(revrate,0,revdsum,-1,0);														//?? should update to bindingradiusprob
				simLog(sim,2,"   unbinding radius if dt were 0: %g\n",unbindingradius(pgem,0,revdsum,bindradrev)); }*/}


//?? Current task: only worry about output of what's actually happening first, then worry about getting input parameters
//?? converted to the correct internal parameters.  For output, goal is to say that the numerical output is equivalent
//?? to each of the continuous-time models, and then give those model parameters.

		if(vflag && order==2 && rxnreactantstate(rxn,statelist,1)) {			// reaction rate details
			ms1=statelist[0];
			ms2=statelist[1];
			i1=rxn->rctident[0];
			i2=rxn->rctident[1];
			...
			if(step>0 && (!rev || revrxn->rparamt!=RPconfspread || revrxn->rparamt!=RPbounce)) {					// a normal bimolecular reaction
				...
				simLog(sim,1,"   diffusion-limited fraction (chi): %g\n",chi);
	...
	if(rxnss) {
		for(r=0;r<rxnss->totrxn;r++) {
			...
			if(rxn->prob<0 || rxn->prob>1) {
				...
				error++; }
			else if(rxn->prob<1 && rxn->rparamt!=RPconfspread && rxn->rparamt!=RPbounce) {		//?? This warning can hopefully be removed
			...
			if(rxn->tau<5*sim->dt) {
				...
				warn++; }}}
										//?? Should check for whether requested diff-limit is achieved
...
int rxnsetrate(simptr sim,int order,int r,char *erstr) {
	...
	if(rxn->disable) {															// disabled reaction
		...
		else if(rparamt==RPunbindrad) rxn->bindrad2=bindingradius(rate3,sim->dt,dsum,rparam,0);			//?? should update to bindingradiusprob
		else if(rparamt==RPbounce) rxn->bindrad2=bindingradius(rate3,sim->dt,dsum,rparam,0);				//?? should update to bindingradiusprob
		else if(rparamt==RPratio) rxn->bindrad2=bindingradius(rate3,sim->dt,dsum,rparam,1);					//?? should update to bindingradiusprob
		else if(rparamt==RPpgem) rxn->bindrad2=bindingradius(rate3*(1.0-rparam),sim->dt,dsum,-1,0);	//?? should update to bindingradiusprob
		else if(rparamt==RPpgemmax || rparamt==RPpgemmaxw) {
			rxn->bindrad2=bindingradius(rate3,sim->dt,dsum,0,0);																			//?? should update to bindingradiusprob
			unbindrad=unbindingradius(rparam,sim->dt,dsum,rxn->bindrad2);
			if(unbindrad>0) rxn->bindrad2=bindingradius(rate3*(1.0-rparam),sim->dt,dsum,-1,0); }			//?? should update to bindingradiusprob
		else rxn->bindrad2=bindingradiusprob(rate3,sim->dt,dsum,-1,0,rxn->difflimit,&rxn->prob);		//?? ERROR: existing prob ignored
...
double rxncalcrate(simptr sim,int order,int r,double *pgemptr) {
	...
	if(order==0) {																	// order 0
		...
		if(rxn->bindrad2<0 || rxn->prob<0) ans=0;
			...
			if(rparamt==RPpgem || (rparamt==RPbounce && rpar>=0) || rparamt==RPpgemmax || rparamt==RPpgemmaxw || rparamt==RPratio || rparamt==RPoffset || rparamt==RPfixed) {
				...
				ans=numrxnrateprob(step,a,bval,prob); }
			else
				ans=numrxnrateprob(step,a,-1,prob);
			ans/=sim->dt;
			if(i1==i2) ans/=2.0;
	...
	if(pgemptr) {
		if(rxn->nprod!=2 || findreverserxn(sim,order,r,&o2,&r2)==0) *pgemptr=-1;
			...
			prob=rxnr->prob;
			*pgemptr=1.0-numrxnrateprob(step,a,-1,prob)/numrxnrateprob(step,a,bval,prob); }}
...
int RxnSetValue(simptr sim,const char *option,rxnptr rxn,double value) {
	...
	else if(!strcmp(option,"difflimit")) {
		if(value>=1) er=4;
