...
int simreadstring(simptr sim,ParseFilePtr pfp,const char *word,char *line2) {
	...
	else if(!strcmp(word,"reaction_difflimit")) {			// reaction_difflimit
		itct=strmathsscanf(line2,"%s %mlg",varnames,varvalues,nvar,rname,&flt1);
		CHECKS(itct==2,"reaction_difflimit format: rname value");
		CHECKS(flt1!=0 && flt1<1,"diffusion-limit value must be between 0 and 1 (or -1 to disable)");
		rxn=NULL;
		r=readrxnname(sim,rname,&order,&rxn,NULL,1);
		if(r>=0) {
			CHECKS(order==2,"diffusion-limit is only allowed for order 2 reactions");
			RxnSetValue(sim,"difflimit",rxn,flt1); }
		r=readrxnname(sim,rname,&order,&rxn,&vlist,2);
		CHECKS(r!=-2,"out of memory reading reaction name");
		if(r>=0) {
			CHECKS(order==2,"diffusion-limit is only allowed for order 2 reactions");
			for(i=0;i<vlist->n;i++)
				RxnSetValue(sim,"difflimit",(rxnptr) vlist->xs[i],flt1);
			ListFreeV(vlist); }
		r=readrxnname(sim,rname,&order,&rxn,NULL,3);
		CHECKS(r!=-2,"out of memory reading reaction name");
		if(r>=0) {
			CHECKS(order==2,"diffusion-limit is only allowed for order 2 reactions");
			RxnSetValue(NULL,"difflimit",rxn,flt1); }
		CHECKS(rxn,"unrecognized reaction name");
		CHECKS(!strnword(line2,3),"unexpected text following reaction_probability"); }
