...
int emuinputparams(emuptr emu) {
	...
	printf("Enter reaction algorithm: (A)ndrews-Bray, (L)ambda-rho, reactive (H)ard sphere: ");
	scanf("%s",string);
	...
	if(emu->algorithm=='l' || emu->algorithm=='h') {
...
int emumakervect(emuptr emu) {
	...
	if(!reversible) dr=10.0*bindrad/npts;
		...
		dr=bindrad*(unbindrad/bindrad+3.0)/npts;
...
int emustepbystep(emuptr emu) {
	...
	double numbindrad,numbindradprob,numprob,chi,rxnrate,numchi,numbindradchi;
	char string[STRCHAR];
	double *r,*rdfa,*rdfd;
	...
	if(unbindrad>=0)
		chi=rxnrate/(4*PI*difc*bindrad*unbindrad/(unbindrad-bindrad));
	else
		chi=rxnrate/(4*PI*difc*bindrad);
...
int emusteadystate(emuptr emu) {
	int npts,it,i,dim;
	double flux,fluxold,dt,numflux,numfluxprob,bindrad,unbindrad,rmsstep,rxnprob,difc;
	double numbindrad,numbindradprob,numprob;
	char string[STRCHAR];
	double *r,*rdfa,*rdfd;
	...
	printf("emulator binding radius: %g\n",bindrad);
	numbindrad=bindingradius(flux/dt,dt,difc,unbindrad,0);
	printf("computed binding radius from bindingradius(%g,%g,%g,%g,0): %g\n",flux/dt,dt,difc,unbindrad,numbindrad);
	numprob=rxnprob;
	numbindradprob=bindingradiusprob(flux/dt,dt,difc,unbindrad,0,-1,&numprob);
	printf("computed binding radius from bindingradiusprob(%g,%g,%g,%g,0,-1,%g): %g\n",flux/dt,dt,difc,unbindrad,numprob,numbindradprob);
...
/* emulate */
int emulate(emuptr emu) {
	int npts,i;
	double unbindrad,flux;
	double *r,*rdfa;
	...
	// compute flux
	if(emu->dim==1)
		flux=xdfsteadystate(r,rdfa,emu->rdfd,npts,0,unbindrad,1.0,0,emu->epsilon);
	else if(emu->dim==3)
		flux=rdfsteadystate(r,rdfa,emu->rdfd,npts,emu->rmsstep,emu->bindrad,unbindrad,emu->epsilon,emu->rxnprob);
...
int emugraphnumrxnrate(emuptr emu) {
	...
	for(i=0;i<npts;i++) {
		x[i]=(i==0)? xlo : x[i-1]*xstep;
		y[cols*i+0]=numrxnrateprob(x[i],a,b,0.5);
		...
		y[cols*i+9]=numrxnrateprob(x[i],a,b,1); }

	if(!gnu) gnu=gnuWindow();
	gnuSetScales(gnu,0,x[npts-1],y[0],y[0]);
...
int main(void) {
	...
	while(selection!=9) {
		...
		printf(" (6) Graph numerical reaction rate\n");
		printf(" (7) Compute steady state flux without graphics\n");
		printf(" (9) Quit\n");
		printf("> ");
		scanf("%i",&selection);
